...
impl TemplateEngine {
    ...
    fn init_template_engine() -> Result<Handlebars, TemplateError> {
        ...
        handlebars
            ...
            .map_err(|e| TemplateError {
                msg: format!("Failed to register templates dir {}", e),
            })?;

        let i18_reg = TemplateEngine::load_i18_reg(&path).map_err(|e| TemplateError {
            msg: format!("Failed to load i18 file: {}", e),
        })?;
    ...
    fn load_i18_reg(path: &Path) -> Result<HashMap<String, String>, IError> {
        ...
        let reg: HashMap<_, _> = conf.iter().flat_map(|(_, prop)| prop.clone()).collect();

        info!("i18 registry is loaded, size {}", reg.len());
        Ok(reg)
    }

    pub fn render<T>(&self, template_name: &str, data: T) -> Result<String, TemplateError>
    where
        T: Serialize + std::fmt::Debug,
    {
        ...
        self.handlebars
            ...
            .map_err(|e| TemplateError { msg: e.to_string() })
    }
}
